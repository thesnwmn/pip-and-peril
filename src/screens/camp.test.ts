...
describe('activity buttons', () => {
  ...
  it('six buttons span full canvas width', () => {
    const btn0 = getActivityButtonRect(0)
    const btn5 = getActivityButtonRect(5)
    expect(btn0.x).toBe(0)
    expect(btn5.x + btn5.w).toBeGreaterThanOrEqual(LOGICAL_W - 4)  // allow rounding
  })
  ...
  it('all buttons have the same height', () => {
    const heights = [0, 1, 2, 3, 4, 5].map((i) => getActivityButtonRect(i).h)
