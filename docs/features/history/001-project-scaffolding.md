...
**Date:** 2026-06-01 · **PR:** (pending merge)

### What was built

Complete TypeScript + Vite + Vitest scaffolding replacing the POC-only approach. Vite provides dev
server with HMR and production bundling; TypeScript brings type safety; Vitest provides unit testing
in jsdom environment. POCs preserved as static HTML in `public/poc/`, served from `/poc/` paths in
production build. All five npm commands implemented: `dev`, `build`, `test`, `typecheck`. GitHub
Pages deploy workflow updated to build and deploy from `dist/`.
...
- Production build verified: root serves game HTML, `/poc/index.html` serves POC listing,
  `/poc/tilemap/` serves tilemap POC, all POCs accessible as static HTML

### Play-test

1. `npm run dev` — start the dev server at http://localhost:5173
2. Visit http://localhost:5173/ — should show a dark canvas with "Pip & Peril" text (canvas
   rendering via src/main.ts)
3. Visit http://localhost:5173/poc/index.html — should show the POC index page with links to all
   5 POCs
4. Visit http://localhost:5173/poc/tilemap/index.html (or build, then /poc/tilemap/) — should
   show the tile map POC with grid and controls
5. Run `npm run build` — should complete successfully and create `dist/` folder with all files
   including POCs
6. Serve `dist/` folder with any static server (e.g. `python3 -m http.server`) — all paths
   should work as in steps 2-4
